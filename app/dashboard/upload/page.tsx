...
import { FileText, Download, Eye, Upload, Loader2, Plus, Calendar, BookOpen } from "lucide-react"
...
export default function YourUploadsPage() {
  ...
  // Fetch a presigned URL from our API and open it in a new tab (view)
  ...
  // Fetch a presigned URL and trigger a file download
  const handleDownload = async (noteId: string, title: string) => {
    ...
    try {
      ...
      // Fetch the file as a blob so we can force a download with the correct filename
        ...
        {loading ? (
          ...
          <div className="divide-y divide-[#F1F5F9]">
            {uploads.map((file) => (
              <div
                key={file.id}
                className="flex items-center gap-4 p-4 sm:p-5 transition-all duration-150 hover:bg-[#F8FAFC] group"
              >
                {/* File icon */}
                ...
                <div className="flex items-center gap-1 shrink-0 opacity-60 group-hover:opacity-100 transition-opacity">
                  ...
                  {file.fileUrl && (
                    <button
                      ...
                      disabled={actionLoading[file.id] === "view"}
                  ...
                  {file.fileUrl && (
                    <button
                      ...
                      disabled={actionLoading[file.id] === "download"}
